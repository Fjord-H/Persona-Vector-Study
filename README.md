...
>
> **Conclusion:** The v2 pipeline is methodologically sound and the infrastructure is reusable. The primary claim — activation methods outperform surface-form baselines — is not supported on content-varying data. On the phrasing/register-varying data (sub-test B): TF-IDF and base-model activation methods have comparable threshold-free discrimination (AUROC 0.920 vs. 0.872–0.979); the instruct model maintains better accuracy under the distribution shift (95.8% vs. 85.4%), though this gap is directional and below statistical significance at N=24 pairs. The "tone-invariance" framing overstates what sub-test B tests: it also varies register and length, and the degradation pattern is register-driven rather than tone-driven. The honest claim is that the instruct model is more robust under A→B distribution shift; the mechanism is unresolved. See `Pipeline_v2/` for full pipeline code and `method_comparison_results.csv` for all reported numbers.
>
