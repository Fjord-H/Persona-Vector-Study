...
>
> **The clearest illustration of this study's finding, in two numbers:** the logistic-regression probe on `llama-3.2-3b/raw` scores AUROC 0.990 on sub-test B — near-perfect ranking, harmful items score higher than neutral items almost without exception — but only 56.2% accuracy, barely above chance. The ranking is essentially intact; the threshold that turns a ranking into a decision is badly placed for this distribution. That single pair of numbers is the whole thesis of what follows: activation-based detectors are not blind on distribution-shifted data, they are miscalibrated on it.
>
> **Sub-test B neutral arm (phrasing-varied, content-fixed, N=24; see register-shift note in analysis paragraph below):** The picture changes by model and formatting:
>
> | model | formatting | method | sub-test A | sub-test B | Wilson 95% CI (item) | A/B gap |
> |---|---|---|---|---|---|---|
...
>
> †length-flagged; pooling variant correlated with prompt length. ‡Threshold artifact, not score inversion — see AUROC note below. llama-3.2-3b-instruct (77.1% sub-test B, 21.6 pp gap) omitted from primary table — extraction manifest not in downloaded caches, provenance unverified; treat as supplementary context only.
>
> **Key finding (updated after fourth external review):** Leading with the threshold-free metric: TF-IDF AUROC on sub-test B (combined arm, 48 neutral + 6 harmful, N=54) = **0.920** — within the base-model content_pole range of 0.872–0.979. At the accuracy level: qwen2.5-1.5b-instruct holds 95.8% (46/48 correct, 2 errors; pair-level Wilson CI [74.2%, 97.7%]) vs. TF-IDF's 85.4% (18/24 pairs, 7 errors; pair-level Wilson [55.1%, 88.0%]) — directional but not significant (McNemar p = 0.062–0.180, not significant at 0.05; pair-level CIs overlap). Error-rate scaling: TF-IDF's error rate grows 2.2× from sub-test A to sub-test B (6.6% → 14.6%), content_pole's grows 1.6× (2.6% → 4.2%). The base model (`qwen2.5-1.5b`) collapses to 14.6%.
>
> **What sub-test B actually tests:** A fourth-round investigation compared sub-test A neutral (N=300) against sub-test B neutral (both phrasing variants, N=48) on form, length, and style. Topic categories are identical (all 6 buckets match perfectly). But the writing register differs sharply: sub-test A neutral is 73% imperative-start, mean 66 chars, 0% polite markers; sub-test B calm is 0% imperative, 92% question-start, 42% polite markers, mean 122 chars; sub-test B hostile is 4% imperative, 79% demand-register, 100% personal pronouns ("my"/"I"/"me"), mean 112 chars. The calm/hostile tone split is not what separates B from A — both phrasing types differ from A neutral in register. TF-IDF's 4 calm / 3 hostile sub-test B errors confirm the degradation is register-driven, not tone-driven.
>
> **Honest reframing:** Activation methods degrade less under the A→B register/distribution shift than TF-IDF (error rate ×1.6 vs. ×2.2). The controlled base/instruct comparison (same architecture, same parameters, only post-training differs) shows qwen-instruct is more robust under this shift than qwen-base. Whether this reflects instruction tuning producing more *register*-invariant representations or better threshold calibration is not resolved. The "tone-invariance" framing is unsupported by the data — sub-test B varies register and length in addition to phrasing tone. Form confound check: both harmful (80%) and neutral (73%) items in sub-test A start with imperatives; removing procedural form tokens from the stopword list does not further reduce TF-IDF's accuracy (0.0 to +3.9 pp on top of stopword removal). **Note on the two TF-IDF baseline numbers in this document:** the headline 93.4% (table above, single-fit) uses `TfidfVectorizer` with no stopword removal at all (sklearn default: every token kept). The ablation's 90.8% baseline is a *different* fit that first applies sklearn's standard English stopword list (removing "the," "a," "is," etc.) before the procedural-token ablation is layered on top — the two numbers are not the same measurement and should not be compared to each other directly; the ablation's internal comparison (90.8% → 90.8%/94.7%) is the relevant one, and it shows procedural-token removal on top of standard stopwords doesn't hurt further. TF-IDF's sub-test A accuracy is content-based under both protocols.
>
> **AUROC, probe, and diagnostics (second–fourth external review, September 2026):** Base-model content_pole combined-arm AUROC (N=54, neutral+harmful) ranges 0.872–0.979. TF-IDF combined-arm AUROC on the same 54 items: **0.920** (sub-test A test split AUROC: 0.964). TF-IDF's AUROC on sub-test B is within the base-model activation-method range — the discrimination ability is comparable; the difference in accuracy is a threshold-calibration artifact. The qwen2.5-1.5b/chat 14.6% result is a threshold-transfer artifact: the midpoint threshold calibrated on sub-test A's validation set does not transfer to sub-test B's shifted distribution (the 85.4% false-positive rate on neutral items is arithmetically 1 − 14.6%, a restatement, not independent evidence). Probe (updated after fourth review — standardized features, C tuned on val): gpt2-medium/raw improved dramatically from 10.4% to 50.0% accuracy (AUROC 0.851) after StandardScaler + C=0.01; the original 10.4% was caused by outlier activation dimensions penalized unevenly by untuned L2 — the reviewer's prediction was correct. For qwen2.5-1.5b/chat: standardized probe accuracy 60.4% (AUROC 0.965, vs. 70.8% unstandardized); for llama-3.2-3b/raw: 56.2% (AUROC 0.990, vs. 62.5% unstandardized) — accuracy dips slightly after standardization for these models, but AUROC confirms the signal is present and the threshold still does not transfer cleanly. Unstandardized probe C=1.0 default numbers (70.8%, 62.5%, 10.4%) are replaced by the standardized+tuned results (60.4%, 56.2%, 50.0%) for future reference; AUROC (0.965, 0.990, 0.851) is the cleaner summary. Pipeline provenance: manifests for all four locally cached models show git commit 5e47591, clean working tree. Wilson 95% CIs use the closed-form Wilson score interval (z=1.96); table CIs are item-level; pair-level CIs are in the Key finding text.
>
> **Does threshold recalibration recover the gap? (fifth external review, September 2026):** If the accuracy drop is threshold-transfer failure rather than representation failure, refitting the decision threshold on a small labeled sample from sub-test B should recover most of the lost accuracy. Tested on the two clearest failure cases. Calibration design: 6 randomly chosen neutral pairs (12 items, seed=42) + all 6 clean harmful items = 18-item slice. Held-out: remaining 18 neutral pairs (36 items). Threshold refit by grid search over calibration scores (no weights or directions changed). Results: `qwen2.5-1.5b/chat` content_pole — original threshold (0.0) gives 13.9% accuracy on the held-out 36 items; recalibrated threshold (+15.77, fit on the 18-item slice) gives **100.0%** (36/36), Wilson CI [90.4%, 100.0%]; oracle threshold (fit on the full 54-item arm) also gives 100%, and the recalibrated and oracle thresholds are identical. `llama-3.2-3b/raw` probe — original threshold (0.5) gives 58.3% (21/36); recalibrated threshold (0.866) gives **91.7%** (33/36), Wilson CI [78.2%, 97.1%]; oracle gives 97.2% (35/36). Both recalibrated thresholds nearly match their respective oracles. The ranking is correct; only the threshold placement was wrong. **Interpretation:** 18 labeled items from the target distribution were sufficient to recover nearly all of the lost accuracy in both cases. The practical implication is that the recalibration cost is low — a small held-out sample from the deployment distribution, not a full retraining run. Caveat: the calibration slice itself is N=18 and the held-out is N=36; these results are internally consistent and striking, but small-N results can be sensitive to which pairs happen to land in which slice. Script: `analysis/review5_threshold_recalibration.py`.
>
...
>
> **Known gaps:** The pipeline saves aggregate metrics but not per-item predictions; this is why McNemar's test for the instruct models returned a p-value range rather than an exact value (per-item predictions are not recoverable from the saved state). Prediction arrays should be persisted per-run in future iterations to avoid this. A logistic-regression probe was evaluated on sub-test B (at the layer/pooling selected by content_pole), but not run as a full fourth method through sub-test A's validation-sweep pipeline with its own layer selection; doing so could still change the sub-test A picture. AUROC/probe/Wilson coverage is incomplete for qwen2.5-1.5b-instruct (corrupted shard, 182 missing IDs) — its accuracy figures are from the original full Kaggle run. Results are reported as accuracy with Wilson and bootstrap CIs, not recall/TPR at a fixed false-positive rate, which is what a deployment decision would actually need.
>
> **Conclusion:** The v2 pipeline is methodologically sound and the infrastructure is reusable. The primary claim — activation methods outperform surface-form baselines — is not supported on content-varying data. On the phrasing/register-varying data (sub-test B): TF-IDF and base-model activation methods have comparable threshold-free discrimination (AUROC 0.920 vs. 0.872–0.979). Because AUROC is comparable, the accuracy gap (95.8% vs. 85.4%) is most likely explained by *which fixed threshold happens to transfer better* to the shifted distribution, not by activation-space representations being inherently more distribution-robust — the same underlying signal is present in both, and TF-IDF was never given the chance to recalibrate its threshold on sub-test B either. The gap is also directional and below statistical significance at N=24 pairs. The "tone-invariance" framing overstates what sub-test B tests: it also varies register and length, and the degradation pattern is register-driven rather than tone-driven. The defensible claim left standing is narrow: under one A→B distribution shift, one instruct model's default threshold happened to transfer better than TF-IDF's; this is not evidence that activation-space methods are more robust in general, and the mechanism is unresolved. The one genuinely positive finding is the recalibration result: 18 labeled items from sub-test B were sufficient to recover accuracy from 13.9% → 100% (qwen content_pole) and 58.3% → 91.7% (llama probe), confirming that the representations carry usable signal and the gap was entirely threshold placement. See `Pipeline_v2/` for full pipeline code and `method_comparison_results.csv` for all reported numbers.
>
