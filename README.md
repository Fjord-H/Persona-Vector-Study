...
>
> **AUROC, probe, and supplementary analysis (second and third external review, September 2026):** For all base-model content_pole results, combined-arm AUROC (neutral+harmful, N=54) ranges 0.872–0.979 — the score direction is consistently correct. The qwen2.5-1.5b/chat 14.6% result is a threshold-transfer artifact, not an inversion: the midpoint threshold calibrated on sub-test A's validation set does not transfer to sub-test B's distribution, where hostile phrasing shifts neutral item scores above it (the resulting 85.4% false-positive rate on neutral items is arithmetically 1 − 14.6%, a restatement of the same error, not independent evidence). A logistic-regression probe trained on the same layer and pooling as content_pole (sub-test A train split, N=392) and evaluated on sub-test B's neutral arm reaches 70.8% [56.8%, 81.8%] vs. content_pole's 14.6% [7.2%, 27.2%] on qwen2.5-1.5b/chat, and 62.5% vs. 43.8% on llama-3.2-3b/raw — the probe is more tone-robust because it learns a boundary in the activation space rather than inheriting a fixed midpoint threshold. The gpt2-medium probe result (10.4%) is also threshold-transfer failure, not overfitting: the probe's decision boundary intercept does not transfer to sub-test B's distribution; if p>>n overfitting were the cause, the largest model (llama-3.2-3b, hidden dim 3072) would fail worst — it does not (probe 62.5%); probe C was not tuned on val (sklearn default C=1.0); features were not standardized. TF-IDF coefficient inspection: top harmful-weighted tokens are generic request vocabulary ("how to", "step", "write") rather than hostile-tone markers; sub-test B errors split nearly equally between calm (4/24) and hostile (3/24) variants, so TF-IDF's degradation appears content-driven, not tone-driven. Pipeline provenance: extraction manifests for all four locally cached model variants (gpt2-medium, qwen2.5-1.5b, qwen2.5-1.5b-instruct, llama-3.2-3b) show git commit 5e47591 with clean working tree — the headline claim's activations are verified pipeline-consistent. llama-3.2-3b-instruct activations came from an earlier `pipeline-v2` Kaggle notebook; that notebook's manifest is not in the downloaded caches — its reported accuracy (77.1%) is the original Kaggle number, provenance unverified, treat as indicative. Wilson 95% CIs use the closed-form Wilson score interval (z=1.96); table CIs are item-level; pair-level CIs (resampling at the 24-pair level) are reported in the Key finding text above. Instruct-model accuracy figures come from the original full Kaggle run; base-model figures and AUROC/probe numbers are verified locally (qwen2.5-1.5b-instruct's chat cache has a corrupted shard — 182 missing IDs — so AUROC/probe could not be computed for that model).
>
...
>
> **Known gaps:** A logistic-regression probe was evaluated on sub-test B (at the layer/pooling selected by content_pole — see AUROC note above), and its own accuracy numbers are reported, but it was not run as a full fourth method through sub-test A's validation-sweep pipeline with its own layer selection; doing so could still change the sub-test A picture. AUROC/probe/Wilson coverage is incomplete for the two instruct models (see corrupted-shard note above) and for `llama-3.2-3b-instruct` (not present in the local extraction cache used for this analysis) — their accuracy figures are the original Kaggle numbers, without the new diagnostics. Results are reported as accuracy with Wilson and bootstrap CIs, not recall/TPR at a fixed false-positive rate, which is what a deployment decision would actually need.
>
