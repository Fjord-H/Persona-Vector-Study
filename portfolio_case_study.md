...
**Threshold recalibration — a closing positive result (fifth external review).** If the accuracy drop is a calibration failure, refitting the threshold on a small labeled sample from the target distribution should recover it. Tested on the two clearest failures: `qwen2.5-1.5b/chat` content_pole (AUROC 0.910, 14.6% accuracy) and `llama-3.2-3b/raw` probe (AUROC 0.990, 56.2% accuracy). Calibration design: 6 neutral pairs (12 items, seed=42) + all 6 clean harmful items = 18-item slice from sub-test B; held-out: remaining 18 neutral pairs (36 items). No weights or directions changed — only the decision threshold was refit by grid search on calibration scores. Results: qwen content_pole 13.9% → **100.0%** (36/36, Wilson [90.4%, 100.0%]); llama probe 58.3% → **91.7%** (33/36, Wilson [78.2%, 97.1%]). Oracle threshold (fit on full 54-item arm) gives 100.0% and 97.2% — the recalibrated thresholds closely match. 18 labeled items from the deployment distribution were sufficient to recover nearly all of the accuracy gap in both cases. The signal was always there; the threshold placement was the entire problem.
