...
†length-flagged result, treat with care. *pair-level Wilson CI (resampling at the 24-pair level); item-level CI [86.0%, 98.9%]. All other table CIs are item-level.

The instruct model with chat formatting holds 95.8% accuracy on tone-varying pairs — 10 points above TF-IDF on the same items. The base model (same architecture, same size, no instruction tuning) drops to 14.6%. That's the controlled comparison this kind of claim requires — same model family, same parameter count, only the post-training differs — and it's consistent with instruction tuning producing more tone-invariant representations. It is not proof: N=24 pairs, one model family, no mechanistic account of why. I state it as suggestive and preliminary, not a general claim.
