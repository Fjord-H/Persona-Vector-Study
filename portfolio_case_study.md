...
The headline is better stated as: TF-IDF degrades more under A→B distribution shift than the instruct activation method. Threshold-free comparison (AUROC): TF-IDF 0.920 on the 54-item combined arm; base-model content_pole range 0.872–0.979. The discrimination ability is comparable by AUROC; the gap in accuracy (95.8% vs. 85.4%, 2/48 vs. 7/48 errors) is real but not significant (McNemar p = 0.062–0.180; pair-level CIs overlap). Error-rate scaling: TF-IDF's error rate grows 2.2× (6.6%→14.6%), content_pole's 1.6× (2.6%→4.2%).
